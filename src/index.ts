...
class WebhookHandler {
  ...
  private async handleEvent(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (activities.length === 0) {
        res
          .status(200)
          .json({status: "ok", message: "No activities to save"});
        return;
      }

      res
        .status(200)
        .json({status: "ok", activitiesSaved: activities.length, activities});
    } catch (error) {
      ...
      res
        .status(500)
        .json({error: "Internal server error", message: toMessage(error)});
    }
  }
  ...
  async fetchAndSaveActivities(athleteId: number) {
    ...
    const {activities, refreshedTokens} =
      await stravaClient.getRecentActivities(
        {
...
export const stravaWebhookHandler = onRequest({region: REGION}, (req, res) =>
...
export const fetchAthleteActivities = onRequest(
  ...
  async (req, res) => {
    try {
      ...
      const {activities} =
        await webhookHandler.fetchAndSaveActivities(athleteId);

      res
        .status(200)
        .json({athleteId, activitiesSaved: activities.length, activities});
    } catch (error) {
      if (error instanceof AthleteNotFoundError) {
        res
          .status(404)
          .json({error: "Athlete not found", athleteId: error.athleteId});
        return;
      }
